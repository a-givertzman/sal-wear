...
use std::sync::Arc;
struct AmplitudeSweepResult {
    amplitude: f64,
    frames_to_first_response: Option<u64>,  // первый Some после начала помехи
    frames_to_saturation: Option<u64>,      // когда прирост падает ниже 5% от начального
    peak_dev: f64,                          // максимальное отклонение от чистого тренда
    frames_to_return: Option<u64>,          // сколько кадров после конца помехи до возврата к тренду (dev<5%)
}
...
fn measure_reaction(
    ...
    spectrum_model.add_target_disturbance(SpectralDisturbance {
        ...
        duration_frames: impulse_duration,
    ...
    // ImbContext::new должен принимать window/q как параметры (временная правка)
    let mut i_ctx = ImbContext::new_with_params(
        dbg, conf.analysis.samples_per_rev(), conf.analysis.n_fft(),
        conf.adc.chunk_size, retain.clone(), window, q,
    );
    ...
    for i in 0..total_frames {
        ...
        if let Some(feat) = i_ctx.features.iter().find(|f| f.order_id == "1x") {
            history.push((i, feat.rms.0));
        }
    }

    // Первое срабатывание после начала помехи
    let frames_to_first_response = history.iter()
        .find(|(f, _)| *f >= impulse_start)
        .map(|(f, _)| f - impulse_start);

    // Отклонения внутри окна помехи
    let impulse_points: Vec<(u64, f64)> = history.iter()
        .filter(|(f, _)| *f >= impulse_start && *f < impulse_start + impulse_duration)
        .cloned().collect();

    let mut peak_dev = 0.0f64;
    for (f, v) in &impulse_points {
        let clean = A0 * trend_multiplier.powf(*f as f64) * amplitude_factor;
        let dev = (v - clean).abs() / clean;
        if dev > peak_dev { peak_dev = dev; }
    }

    // Кадр насыщения — где прирост падает ниже 5% от начального
    let frames_to_saturation = if impulse_points.len() > 2 {
        let initial_growth = (impulse_points[1].1 - impulse_points[0].1).abs();
        impulse_points.windows(2)
            .find(|w| (w[1].1 - w[0].1).abs() < initial_growth * 0.05)
            .map(|w| w[1].0 - impulse_start)
    } else { None };

    // Возврат к тренду после конца помехи
    let after_points: Vec<(u64, f64)> = history.iter()
        .filter(|(f, _)| *f >= impulse_start + impulse_duration)
        .take(30).cloned().collect();
    let frames_to_return = after_points.iter()
        .find(|(f, v)| {
            let clean = A0 * trend_multiplier.powf(*f as f64) * amplitude_factor;
            (v - clean).abs() / clean < 0.05
        })
        .map(|(f, _)| f - (impulse_start + impulse_duration));

    AmplitudeSweepResult {
        amplitude, frames_to_first_response, frames_to_saturation, peak_dev, frames_to_return,
    }
}

#[test]
fn amplitude_sweep_test() {
    DebugSession::new().filter(LogLevel::Debug).init();
    let dbg = Dbg::own("AmplitudeSweep-test");
    const F_SAMPLE: i32 = 320_000;
    ...
    let amplitudes = [0.5, 1.0, 1.2, 1.5, 2.0, 3.0, 5.0];
    let window = 600; 
    let q = 1e-12;

    println!("{:<10} {:<15} {:<15} {:<12} {:<15}", "amp", "first_resp", "saturation", "peak_dev,%", "return");
    for &amp in &amplitudes {
        let r = measure_reaction(amp, window, q, &dbg, &conf, hours_8_in_frames);
        println!(
            "{:<10} {:<15} {:<15} {:<12.1} {:<15}",
            amp,
            r.frames_to_first_response.map(|v| v.to_string()).unwrap_or("-".into()),
            r.frames_to_saturation.map(|v| v.to_string()).unwrap_or("-".into()),
            r.peak_dev * 100.0,
            r.frames_to_return.map(|v| v.to_string()).unwrap_or("-".into()),
        );
    }
}
