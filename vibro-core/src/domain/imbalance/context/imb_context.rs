...
impl ImbContext {
    ...
    /// - `fft_turns` - Количество полных оборотов для достижения нужного разрешения (из конфига).
    pub fn new(parent: impl Into<String>, n_rev: usize, fft_turns: usize, retain: Arc<Retain>) -> Self {
        ...
        //      - Угловое разрешение (Сэмплов на оборот): Берем из конфига (`n_rev`).
        ...
        //        `ΔOrder = 1 / 32 ≈ 0.03125` порядка.
        //        Это позволит детектору легко отличить, например, дисбаланс ротора (1.0x) от дефекта сепаратора FTF (обычно около $0.38x ... 0.42x).
        let capacity = fft_turns * n_rev;
        // Размер буфера FFT
        let n_fft = OrderSpectrum::<Pass>::fft_buffer_size();
